        ...
        if position[feature_name1] == i:
            ...
            if i == 0:
                ...
                if len(ca_range.keys()) == 0:
                    ...
                    full_grid = np.c_[
                        full_grid, np.tile(df[df.columns[i]], ca_range[feature_name1] * ca_range[feature_name2])]
    return full_grid, ca_range, [feature_name1, feature_name2]
...
def mean_prediction_from_grids(df, model, feature_names, resolution, target_names, encoder_dict, input_ranges):
    ...
    """
    grid, ca_range, feature_n = feature_grid_total(df, feature_names, resolution, encoder_dict, input_ranges)
    model_output = prediction_from_grid(model, grid)
    if target_names:
...
def run_model(n_clicks, epochs, data, target, normalize, use_features, learning_rate, hidden_layers, num_layers,
    ...
    if n_clicks:
        ...
        loss_fn = define_loss_function(F.cross_entropy) if class_or_reg == "Classification" else define_loss_function(
            F.mse_loss)
        X_train_ = []
        y_train = np.repeat(y_train, len(X_train)*len(X_train[0]))
        for i in range(len(X_train)):
            for j in range(len(X_train[0])):
                build_train_set = []
                for k in range(len(X_train[0])):
                    if j == k:
                        build_train_set.append(X_train[:, j])
                    else:
                        build_train_set.append(np.repeat(X_train[i][k], len(X_train)))
                X_train_.append(np.array(build_train_set).T)
        X_train_ = np.array(X_train_).reshape(-1, len(X_train[0]))
        trained_model, test_accuracy = train_and_evaluate_model(model, X_train_, y_train, X_test, y_test, optimizer,
